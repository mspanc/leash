class Leash::Provider::UserInfoController < Leash::ProviderController
  def info
    render text: "missing_authorization_header", status: :unauthorized and return unless request.headers["Authorization"]
    render text: "missing_authorization_bearer", status: :unauthorized and return unless request.headers["Authorization"].start_with? "Bearer "

    access_token_raw = request.headers["Authorization"].split(" ", 2).last

    render text: "invalid_access_token", status: :forbidden and return unless Leash::Provider::AccessToken.valid?(access_token_raw)
